...
class PluginManager
{
	...
	{
		// Retrieve all currently active plugins from the database
		$activePlugins = Plugin::where( 'is_active', true )->get(); // Use renamed Model class
		...
		foreach ( $this->loadedPluginInstances as $pluginInstance ) {
			try {
				...
				foreach ( $pluginInstance->registerSettings() as $setting ) {
					app( SettingsManager::class )->register( // Use renamed SettingsManager
						$setting['key'],
	...
	public function deactivatePlugin( string $pluginSlug ): Plugin // Return renamed Model class
	{
		...
		// 4. Clear caches and regenerate autoloader
		$this->runComposerDumpAutoload();
		$this->clearCaches();
	...
	public function activatePlugin( string $pluginSlug ): Plugin // Return renamed Model class
	{
		...
		// 5. Clear caches and regenerate autoloader (final step for changes to take effect)
		$this->runComposerDumpAutoload();
		$this->clearCaches();
	...
	public function uninstallPlugin( string $pluginSlug ): void
	{
		...
		// 5. Clear caches and regenerate autoloader one last time
		$this->runComposerDumpAutoload();
		$this->clearCaches();
	}
}
