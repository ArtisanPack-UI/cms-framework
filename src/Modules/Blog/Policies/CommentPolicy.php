...
class CommentPolicy
{
    ...
    public function viewAny( ?Authenticatable $user ): bool
    {
         ...
         */
        $capability = applyFilters( 'ap.cmsFramework.abilities.comments.viewAny', 'comments.view' );
        ...
        if ( applyFilters( 'ap.cmsFramework.abilities.comments.viewAny.public', true ) ) {
            return true;
        }
    ...
    public function view( ?Authenticatable $user, Comment $comment ): bool
    {
        if ( Comment::STATUS_APPROVED === $comment->status
            && applyFilters( 'ap.cmsFramework.abilities.comments.view.public', true, $comment ) ) {
            return true;
        }
        ...
        return $user->can( applyFilters( 'ap.cmsFramework.abilities.comments.view', 'comments.view', $comment ) );
    }
    ...
    public function create( ?Authenticatable $user ): bool
    {
        ...
        if ( null === $user ) {
            return ( bool ) applyFilters( 'ap.cmsFramework.abilities.comments.create.public', true );
        }

        return $user->can( applyFilters( 'ap.cmsFramework.abilities.comments.create', 'comments.create' ) );
    }
    ...
    {
        $canEditAny = $user->can( applyFilters( 'ap.cmsFramework.abilities.comments.update', 'comments.edit', $comment ) );

        if ( $canEditAny ) {
            return true;
        }

        $canEditOwn = $user->can( applyFilters( 'ap.cmsFramework.abilities.comments.updateOwn', 'comments.editOwn', $comment ) );
    ...
    {
        $canDeleteAny = $user->can( applyFilters( 'ap.cmsFramework.abilities.comments.delete', 'comments.delete', $comment ) );

        if ( $canDeleteAny ) {
            return true;
        }

        $canDeleteOwn = $user->can( applyFilters( 'ap.cmsFramework.abilities.comments.deleteOwn', 'comments.deleteOwn', $comment ) );
    ...
    {
        return $user->can( applyFilters( 'ap.cmsFramework.abilities.comments.moderate', 'comments.moderate', $comment ) );
    }
}
