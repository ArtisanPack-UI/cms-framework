 ...
 *
 * @since 2.0.0
 * @package ArtisanPackUI\CMSFramework\Modules\Notifications\Managers
 */
...
class NotificationManager
{
    ...
    public function registerNotification(
         ...
         */
        addFilter('ap.notifications.registeredNotifications', function ($notifications) use ($key, $title, $content, $type, $sendEmail, $metadata) {
            $notifications[$key] = [
                'title' => $title,
                'content' => $content,
                'type' => $type,
    ...
    public function sendNotificationByRole(string $key, string $role, array $overrides = []): ?Notification
    {
        ...
        $userIds = $userModel::whereHas('roles', function ($query) use ($role) {
    ...
    {
        $query = Notification::whereHas('users', function ($q) use ($userId) {
    ...
    protected function filterUsersByPreferences(array $userIds, string $notificationKey): array
    {
        ...
        return $userModel::whereIn('id', $userIds)
            ->whereDoesntHave('notificationPreferences', function ($query) use ($notificationKey) {
                $query->where('notification_type', $notificationKey)
