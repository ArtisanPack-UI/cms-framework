...
test( 'can create notification preference', function (): void {
...
test( 'notification preference has correct fillable attributes', function (): void {
...
test( 'notification preference casts is_enabled to boolean', function (): void {
    $preference = NotificationPreference::factory()->create( [
        'is_enabled' => true,
    ] );
...
test( 'notification preference casts email_enabled to boolean', function (): void {
    $preference = NotificationPreference::factory()->create( [
        'email_enabled' => false,
    ] );
...
test( 'notification preference belongs to user', function (): void {
    $user       = User::factory()->create();
    $preference = NotificationPreference::factory()->create( [
        'user_id' => $user->id,
    ] );
...
test( 'user can have multiple notification preferences', function (): void {
    $user = User::factory()->create();
...
test( 'unique constraint prevents duplicate preferences for same user and type', function (): void {
    $user = User::factory()->create();
...
test( 'factory enabled state creates enabled preference', function (): void {
    $preference = NotificationPreference::factory()->enabled()->create();

    expect( $preference->is_enabled )->toBeTrue();
} );

test( 'factory disabled state creates disabled preference', function (): void {
    $preference = NotificationPreference::factory()->disabled()->create();

    expect( $preference->is_enabled )->toBeFalse();
} );

test( 'factory emailEnabled state enables email notifications', function (): void {
