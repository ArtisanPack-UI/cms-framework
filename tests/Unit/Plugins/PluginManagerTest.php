...
describe('Plugin Discovery', function () {
    it('discovers plugins from filesystem', function () {
    ...
    it('merges database status with discovered plugins', function () {
        $pluginsPath = base_path('plugins');
        File::ensureDirectoryExists($pluginsPath);
        File::copyDirectory($this->testPluginsPath.'/valid-plugin', $pluginsPath.'/valid-plugin');
    ...
    it('returns empty array when plugins directory does not exist', function () {
...
describe('Get Plugin', function () {
    it('retrieves a specific plugin by slug', function () {
        $pluginsPath = base_path('plugins');
        File::ensureDirectoryExists($pluginsPath);
        File::copyDirectory($this->testPluginsPath.'/valid-plugin', $pluginsPath.'/valid-plugin');
    ...
    it('prevents path traversal with invalid slug', function () {
        $plugin = $this->manager->getPlugin('../../../etc/passwd');

        expect($plugin)->toBeNull();
    });

    it('prevents path traversal with dot notation', function () {
...
describe('Manifest Validation', function () {
    ...
    it('validates slug format in manifest', function () {
        $manifest = [
    ...
    it('accepts valid semver versions', function () {
        $manifest = [
            'slug' => 'test-plugin',
            'name' => 'Test Plugin',
            'version' => '1.2.3',
...
describe('Manifest Parsing', function () {
    it('parses valid JSON manifest', function () {
        $pluginsPath = base_path('plugins');
        File::ensureDirectoryExists($pluginsPath);
        File::copyDirectory($this->testPluginsPath.'/valid-plugin', $pluginsPath.'/valid-plugin');
