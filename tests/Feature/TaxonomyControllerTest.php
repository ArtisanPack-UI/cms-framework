...
beforeEach( function () {
	...
	$adminRole = \ArtisanPackUI\CMSFramework\Models\Role::factory()->create( [
		'name' => 'Admin',
		'capabilities' => ['manage_taxonomies'],
	] );
	...
	$this->contentType1 = ContentType::factory()->create( [ 'handle' => 'post' ] );
	$this->contentType2 = ContentType::factory()->create( [ 'handle' => 'page' ] );
} );
...
it( 'prevents unauthorized users from managing taxonomies', function () {
	...
	$response = $this->putJson( "/api/cms/taxonomies/{$this->taxonomy->id}", [
		...
		'content_types' => $this->taxonomy->content_types,
		'handle'        => $this->taxonomy->handle,
	] );
	$response->assertStatus( 403 );
