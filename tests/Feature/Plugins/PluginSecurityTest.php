...
describe( 'Path Traversal Prevention', function (): void {
    it( 'prevents directory traversal in slug parameter', function (): void {
        $plugin = $this->manager->getPlugin( '../../../etc/passwd' );
        expect( $plugin )->toBeNull();
    } );

    it( 'prevents directory traversal with multiple dot segments', function (): void {
        $plugin = $this->manager->getPlugin( '../../vendor/autoload' );
        expect( $plugin )->toBeNull();
    } );

    it( 'prevents directory traversal with encoded characters', function (): void {
        $plugin = $this->manager->getPlugin( '%2e%2e%2f%2e%2e%2fetc%2fpasswd' );
        expect( $plugin )->toBeNull();
    } );

    it( 'prevents directory traversal with null byte injection', function (): void {
        $plugin = $this->manager->getPlugin( "valid-plugin\0../../etc/passwd" );
        expect( $plugin )->toBeNull();
    } );

    it( 'validates slug contains only allowed characters', function (): void {
        // Invalid characters
    ...
    it( 'ensures resolved path is within plugins directory', function (): void {
...
describe( 'XSS Prevention', function (): void {
    it( 'sanitizes plugin name with XSS attempts', function (): void {
        $manifest = [
    ...
    it( 'sanitizes plugin description with XSS attempts', function (): void {
        $manifest = [
            'slug'        => 'xss-desc-test',
            'name'        => 'Test Plugin',
    ...
    it( 'handles malicious author fields', function (): void {
        $manifest = [
...
describe( 'SQL Injection Prevention', function (): void {
    it( 'safely handles single quotes in plugin slug', function (): void {
    ...
    it( 'safely handles SQL injection in plugin name', function (): void {
        $manifest = [
            'slug'    => 'sql-test',
    ...
    it( 'safely queries plugins with special characters', function (): void {
        Plugin::create( [
            'slug'    => 'test-plugin',
            'name'    => 'Test Plugin',
            'version' => '1.0.0',
...
describe( 'Upload Security', function (): void {
    it( 'rejects non-ZIP files', function (): void {
        $phpFile = storage_path( 'app/malicious.php' );
        File::put( $phpFile, '<?php system($_GET["cmd"]); ?>' );

        expect( function () use ( $phpFile ): void {
    ...
    it( 'validates ZIP file integrity', function (): void {
        // Note: This test is skipped because dynamically created ZIP files
        // in PHP don't always get the correct MIME type from mime_content_type()
...
describe( 'Manifest Injection Prevention', function (): void {
    it( 'rejects manifest with code execution attempts', function (): void {
        $manifest = [
            'slug'             => 'code-injection',
            'name'             => 'Test Plugin',
            'version'          => '1.0.0',
    ...
    it( 'validates version format to prevent injection', function (): void {
        $manifest = [
...
describe( 'Permission Checks', function (): void {
    it( 'validates plugin slug format prevents unauthorized access', function (): void {
