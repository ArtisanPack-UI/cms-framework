...
The parser is deliberately generic — sibling `TemplateResolver` and `TemplatePartResolver` closed the same `blocks: []` gap in 2.7.2 ([#274](https://github.com/ArtisanPack-UI/cms-framework/issues/274)) via `ThemeFileBlockParser`, which wraps this parser and prefers visual-editor's editor-shape `BlockMarkupHydrator` when it is installed. See [Templates and Template Parts](Templates.md#block-shape-for-theme-file-entities).
