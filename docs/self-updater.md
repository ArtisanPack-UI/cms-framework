...
## Removing files a release deleted upstream (2.10.0)

Extraction adds and overwrites but can never *delete*, so a release that removes a file upstream — a dead `app/Http/Controllers/*.php` left beside its new `Modules/*` replacement after a restructure — would otherwise orphan the old file on the host, where the autoloader can still find and load the stale class. A release declares those deletions in a **removed-paths manifest** it ships in the archive (default `.artisanpack/removed-paths.json`, a JSON array of repo-relative paths); `applyRemovedPathsManifest()` applies them after a successful update.

- **Every deletion is guarded like an extraction write.** Each path is canonicalized (an absolute path or a `..` traversal is rejected), skipped when `exclude_from_update` protects it, never followed through a symlink, and re-checked for containment under `base_path()` before removal.
- **Critical host files can never be removed.** Even when a manifest names them, `.env`, `.htaccess`, `artisan`, `composer.json` / `composer.lock`, `public/index.php` (and its `.htaccess`), and anything under `.git/` are protected — defence-in-depth over the containment and exclusion guards, since a framework release has no business deleting the front controller or environment file.
- **The manifest is consumed after it applies.** Because extraction is add/overwrite-only, a manifest left in place would survive into later updates; a subsequent release that ships no manifest of its own would then re-apply the stale list and delete files that release deliberately re-introduced. Deleting the manifest after it runs closes that data-loss window — a real deletion set only ever arrives inside the archive being extracted.
- **Best-effort, and opt-out.** An absent or malformed manifest is a no-op, and the whole pass is wrapped so it can never fail a completed update. Set `cms.updates.honor_removed_paths_manifest` to `false` to skip it, or `cms.updates.removed_paths_manifest` to relocate the file.

## Purging orphaned build assets (2.10.0)

Content-hashed Vite bundles under `public/build` accumulate across releases — extraction adds the new hashed files but never removes the superseded ones. After a successful update, `purgeStaleBuildAssets()` deletes any file under the build directory (default `public/build`, config `cms.updates.build_path`) that the current `manifest.json` (or its `.vite/manifest.json` sidecar) does not reference, then prunes the directories it empties. The manifest itself is never a purge candidate, symlinks are never followed, and a missing or unparseable manifest purges nothing rather than guessing. Disable it with `cms.updates.purge_stale_build_assets = false`.
